		...
		config = function()
			...
			vim.api.nvim_create_autocmd("LspAttach", {
				...
				callback = function(event)
							...
							vim.lsp.buf.definition({
								on_list = function(options)
									...
									if prefix then
										...
										local from =
											{ vim.api.nvim_get_current_buf(), vim.fn.line("."), vim.fn.col("."), 0 }
										local items = { { tagname = function_name, from = from } }
										vim.fn.settagstack(vim.api.nvim_get_current_win(), { items = items }, "t")
					...
					map("gd", go_goto_def, "[G]oto [D]efinition")
